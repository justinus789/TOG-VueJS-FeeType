<template>
  <v-app-bar flat class="app-bar-layout">
    <v-app-bar-title class="mt-6">
      <img src="@/assets/svg/BAYU BUANA_LOGO_HORIZONTAL_COLOR.svg" alt="">
    </v-app-bar-title>

    <v-spacer />

    <div id="app-bar-button" class="mt-8 mr-6">
      <v-btn icon size="small" >
        <v-icon> mdi-help-circle </v-icon>
      </v-btn>
      
      <v-btn class="mr-2" icon size="small">
        <v-badge dot color="#FF0000">
          <v-icon> mdi-bell-outline </v-icon>
        </v-badge>
      </v-btn>

      <v-menu location="bottom start">
        <template v-slot:activator="{ props }">
          <v-btn icon class="avatar-btn" v-bind="props">
            <v-avatar image="data:image/png;base64,"></v-avatar>
          </v-btn>
        </template>

        <v-list min-width="200" nav>
          <v-list-item
            title="Patrick Jane"
            subtitle="Administrator" 
          >
            <template v-slot:prepend>
              <v-btn icon class="avatar-btn" v-bind="props">
                <v-avatar image="data:image/png;base64,"></v-avatar>
              </v-btn>
            </template>
          </v-list-item>

          <v-divider></v-divider>

          <v-list density="compact" nav>
            <v-list-item prepend-icon="mdi-account" title="My Profile" />
            <v-list-item prepend-icon="$changePassword" title="Change Password"/>
            <v-list-item prepend-icon="$signOut" title="Sign Out"/>
          </v-list>
        </v-list>
      </v-menu>
    </div>
    
  </v-app-bar>
</template>

<script setup>
// import '@/assets/svg/logout.svg' as logoutIcon;
  //
</script>

<style scoped>
.app-bar-layout{
  width: 100%;
  height: 92px;
  box-shadow: 0px 1px 6px #00000029;
  opacity: 1;
}

.avatar-btn{
  border: 1px solid #DADADA;
  margin-right: 10px;
}
</style>
